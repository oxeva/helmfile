...
RUN mkdir -vp "${HOME}/.config/kustomize/plugin/goabout.com/v1beta1/sopssecretgenerator" && \
    curl -L -o "${HOME}/.config/kustomize/plugin/goabout.com/v1beta1/sopssecretgenerator/SopsSecretGenerator" $(curl -Ls https://api.github.com/repos/goabout/kustomize-sopssecretgenerator/releases/latest | jq -r '.assets[] | select(.name | test("^SopsSecretGenerator_.+_linux_amd64$")) | .browser_download_url') && \
